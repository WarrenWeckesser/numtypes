# This file was copied from numpy/numpy/distutils/conv_template.py and
# updated.
...
def main():
    if len(sys.argv) != 2:
        raise ValueError(f'use: {sys.argv[0]} infile')

    infilename = sys.argv[1]
    fid = open(infilename, 'r')
    base, ext = os.path.splitext(infilename)
    if ext != '.src':
        raise RuntimeError(f'error: input filename extension is not .src')

    outfilename = os.path.basename(base)
